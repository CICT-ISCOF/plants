...
export default class Home extends Component {
	...
	async renderPlantitas(){		
		...
		const renderCategory = () => {
			let plantitas = categoryArray.map((category) => {
				return(
					<TouchableOpacity onPress={()=>{this.viewPlantitas(category.name,category)}}>
							<View  style={styles.plantitas}>
							<Image style={styles.plantitasImage} source={{uri:category.photo_url}} />	
							</View>
							<Text style={styles.plantitasName} >{category.name}</Text>
							<Text style={styles.plantitasSubtitle} >for {category.type}s</Text>
					</TouchableOpacity>
				)
			})
	...
	async renderCategories(){
		...
		const renderCategory = ()=> {	
			let categories = categoryArray.map((category) => {
				return(
					<TouchableOpacity key={category.id} onPress={()=>{this.viewCategories(category.id,category.title)}}>
							<View  style={styles.categroyContainer}>
							<Image    blurRadius={15} style={styles.categoryImage} source={{uri:category.photo_url}} />						
							</View>
	...
	async renderCommonPlantPests(){
		...
		const renderCategory = ()=> {	
			let commonPlantPests = categoryArray.map((category) => {
				return(
					<TouchableOpacity key={category.id} onPress={()=>{this.viewCommonPlantPests(category.id,category)}}>
							<View  style={styles.plantitas}>
							<Image style={styles.plantitasImage} source={{uri:category.photo_url}} />						
							</View>
						<Text style={[styles.plantitasName,{textAlign:'center'}]} >{category.name}</Text>
					</TouchableOpacity>
				)
			})
	...
	async renderPlantDiseaseAndDisorders(){
		...
		const renderCategory = ()=> {	
			let plantDiseaseAndDisorders = categoryArray.map((category) => {
				return(
					<TouchableOpacity key={category.id} onPress={()=>{this.viewPlantDiseaseAndDisorders(category.id,category)}}>
							<View  style={styles.categroyContainer}>
							<Image style={styles.categoryImage} source={{uri:category.photo_url}} />						
							</View>
						<Text style={[styles.categoryName]} >{category.title}</Text>
					</TouchableOpacity>
				)
			})
			this.setState({plantDiseaseAndDisorders : plantDiseaseAndDisorders})
			categoryArray = []
		}
	}
    ...
    render() {
        return (
			...
			<ScrollView>
				<Image style={styles.bigImage} source={require('../../assets/home.jpg')} />
				...
				<Text style={styles.title}>Plantitas and Plantitos</Text>
					...
					<ScrollView horizontal={true} style={styles.categoryScrollView}  showsVerticalScrollIndicator={false}
					showsHorizontalScrollIndicator={false}>
						{this.state.categories}
					</ScrollView>

					<Text style={styles.title}>Common Plant Pests</Text>
					<ScrollView horizontal={true}  showsVerticalScrollIndicator={false} style={[styles.categoryScrollView,{borderTopWidth:0,paddingBottom:30,marginBottom:30}]} 
					showsHorizontalScrollIndicator={false}>
						{this.state.commonPlantPests}
					</ScrollView>

					<Text style={styles.title}>Plant Disease and Disorders</Text>
					<ScrollView horizontal={true}  showsVerticalScrollIndicator={false} 
					showsHorizontalScrollIndicator={false}>
				...
				<View  style={globalStyles.footer}>
                  <View style={[globalStyles.footerButtons,globalStyles.active]}>
						<Entypo name="home" size={26}  color={theme[themeState].footerActiveColor} />
						<Text  style={[globalStyles.footerButtonTexts,{color:theme[themeState].footerActiveColor}]}> Home </Text>
                  </View>

                  <TouchableOpacity   onPress={()=>{this.viewPlantInformation()}} style={globalStyles.footerButtons}>                   
						<MaterialCommunityIcons name="calendar-range" size={26} color={theme[themeState].subTitle} />
						<Text style={globalStyles.footerButtonTexts}> Plant Information </Text>
                  </TouchableOpacity>
