...
import styles from './helpful-tips.style'
	...
	async renderHelpfulTips(){
		...
		const renderTip = ()=> {	
			let helpFulTips = tipsArray.map((tip)=>{
				return(
					<TouchableOpacity style={styles.helpFulTips} onPress={()=>{this.viewTips(tip.items)}}>						
		...
		return (
			<View style={globalStyles.mainContainer}>
				...
				<View  style={globalStyles.footer}>
					<TouchableOpacity style={globalStyles.footerButtons} onPress={()=>{this.Home()}} >
					...
					<TouchableOpacity onPress={()=>{this.viewPlantInformation()}} style={globalStyles.footerButtons}>                   
							<MaterialCommunityIcons name="calendar-range" size={26} color="white" />
					...
					<View  style={[globalStyles.footerButtons,globalStyles.active]}>
							<MaterialCommunityIcons name="lightbulb-on" size={26} color={theme[themeState].footerActiveColor} />
							<Text  style={[globalStyles.footerButtonTexts,{color:theme[themeState].footerActiveColor}]}> Helpful Tips </Text>
					</View>			
              </View>
